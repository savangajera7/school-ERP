...
import { normalizeStudent, getStudentDisplayName } from "@/utils/studentUtils";
...
export default function AttendanceScreen() {
    ...
    if (selectedClassId && selectedSectionId) {
      ...
      .then((response: any) => {
        ...
        if (apiBody?.success && apiBody?.data) {
          const rawData = apiBody.data;
          const searchData = toCamelCaseRow<any>(rawData);
          const studentList = searchData.students || searchData.Students || (rawData as any).Students || [];
          setSearchResponse(studentList.map((s: any) => normalizeStudent(s)));
        } else {
          setSearchResponse([]);
        }
      })
  ...
  }, [selectedClassId, selectedSectionId, searchMutate]);
        ...
        <>
          <PremiumCard noAccent style={{ padding: 16, marginBottom: 14 }}>
            <View className="flex-col gap-5">
              ...
              <View className={`flex-row gap-6 ${isMobile ? "flex-col" : "items-start justify-between"}`}>
                <View className="flex-1">
                  <Text className="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-2">Select Class</Text>
                ...
                <View className={`${isMobile ? "w-full border-t border-gray-100 pt-4" : "w-[200px] border-l border-gray-150 pl-6"}`}>
                  <Text className="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-2">Effective Date</Text>
                  <View className="h-[40px] bg-gray-50 border border-gray-200 rounded-xl justify-center px-4">
                    <Text className="text-[13px] font-black text-gray-800">📅 {date}</Text>
                  </View>
                </View>
              </View>

              <View className="w-full">
                <Text className="text-[10px] font-black text-gray-400 uppercase tracking-wider mb-2">Select Section</Text>
                <View className="flex-row flex-wrap gap-2">
                  {sections.map((sec: any) => (
                    ...
                    >
                      <Text className={`text-[11px] font-bold ${
                        selectedSectionId === sec.sectionID ? "text-indigo-700" : "text-gray-600"
                      }`}>
