...
export default function NotebookScreen() {
  ...
  const { items, isLoading, refetch } = useClassroomContentList("NOTEBOOK");
  ...
  const handleAdd = async () => {
    ...
    try {
      await insertContent.mutateAsync({
        data: {
          noticeTitle: buildContentTitle("NOTEBOOK", title),
          noticeDescription: description,
          noticeFor: "Class",
          addedBy: parseInt(userData?.id ?? "0"),
        }
      });
      showToast("Notebook entry added successfully", "success");
      setIsAdding(false);
      setTitle("");
      setDescription("");
      refetch();
    } catch (error) {
      showToast("Failed to add notebook entry", "error");
    }
  };
    ...
    >
      {isAdding && (
        <Card className="p-4 mb-6">
          <Text className="text-[12px] font-black text-gray-400 mb-2 uppercase">New Entry</Text>
          <TextInput
          ...
          />
          <TextInput
            placeholder="Content / Observations"
            value={description}
          ...
          >
            <Text className="text-white font-black uppercase">Post Entry</Text>
          </TouchableOpacity>
        </Card>
      )}
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<EmptyState title="No notebook entries" message="Records will appear here" />}
          renderItem={({ item }) => (
            <Card className="mb-4 p-4 border-l-4" style={{ borderLeftColor: "#6366F1" }}>
              <View className="flex-row justify-between items-start mb-2">
                ...
                <IconCircle name="notebook" size={32} iconSize={16} />
              </View>
